	...
	Repeat 
		...
		If ($Boo_OK)
			...
			For ($Lon_i; 0; Size of array:C274($tLon_Positions); 1)
				...
				If ($tLon_Tempo_Lengths{Size of array:C274($tLon_Tempo_Lengths)}=0) & ($tLon_Tempo_Positions{Size of array:C274($tLon_Tempo_Lengths)}=$tLon_Tempo_Positions{Size of array:C274($tLon_Tempo_Lengths)-1})
					
					$Boo_OK:=False:C215
					$Lon_i:=Size of array:C274($tLon_Tempo_Positions)
					
					DELETE FROM ARRAY:C228($tTxt_Tempo; $Lon_i; 1)
					DELETE FROM ARRAY:C228($tLon_Tempo_Positions; $Lon_i; 1)
					DELETE FROM ARRAY:C228($tLon_Tempo_Lengths; $Lon_i; 1)
					DELETE FROM ARRAY:C228($tLon_Tempo_Index; $Lon_i; 1)
					
					$Lon_i:=Size of array:C274($tLon_Positions)+1
